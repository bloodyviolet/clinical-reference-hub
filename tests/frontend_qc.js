...
assert(
  appScript
    .replace(/\s+/g, '')
    .includes(
      'fetch(`/api/v1/policies/${encodeURIComponent(policyName)}`)'
    )
);
...
assert(sw.includes("clinical-reference-v30-v2-pcdt-ux-bilingual"));
...
assert(html.includes('/manifest.json?v=30'));
...
assert(
  sw.includes('clinical-reference-v30-v2-pcdt-ux-bilingual')
);
